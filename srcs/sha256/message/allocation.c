...
void append_sha256_padding_bits_to_buffer(
...
void append_sha256_length_to_buffer(
...
#define CURRENT_INDENT 1
  ...
  append_sha256_padding_bits_to_buffer(
      buffer, len > i ? len - i : 0, chunk_size, i <= len
  );
  if (i > len ||
      chunk_size - (len - i) >= (LENGTH_PADDING_BITS_NBR / CHAR_BIT) + 1) {
    append_sha256_length_to_buffer(buffer, chunk_size, origin_message_bits_len);
  }
