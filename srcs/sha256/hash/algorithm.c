...
sha256_hash_t sha256_hash_chunk_round(
  ...
  PRINT("ch = %#010x\n", ch);
  PRINT("sha256_k_table[i] = %#010x\n", sha256_k_table[i]);
  PRINT("message_chunk[i] = %#010x\n", message_chunk[i]);
  const sha256_message_chunk_word_t tmp1 =
  ...
  PRINT("maj = %#010x\n", maj);
  const sha256_message_chunk_word_t tmp2 = S0 + maj;
...
sha256_hash_t sha256_hash_chunk(
  ...
  // TODO const 64
  sha256_message_chunk_word_t schedule[64];

  // TODO const 64
  ft_memcpy(schedule, message_chunk, 16 * sizeof(sha256_message_chunk_word_t));
#if __BYTE_ORDER == __LITTLE_ENDIAN
  buffer_bswap_32(schedule, 16 * sizeof(sha256_message_chunk_word_t));
#endif
